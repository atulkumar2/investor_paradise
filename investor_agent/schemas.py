...
class EntryRouterOutput(BaseModel):
    ...
    intent: Literal[
        ...
        "out_of_scope",
        "prompt_injection",
    ] = Field(description="Classified intent of user's query")

    should_analyze: bool = Field(description=(
...
class StockPerformance(BaseModel):
    ...
    return_pct: float = Field(
        description="Percentage return over the analysis period"
    )
    ...
    volatility: Optional[float] = Field(
        None,
        description=(
            "Daily return standard deviation as percentage "
...
class MarketAnalysisOutput(BaseModel):
    ...
    risk_flags: List[str] = Field(
        default=[],
        description=(
            "Any anomalies or risks detected (e.g., 'RADIOCITY: 838% return - "
    ...
    focus_areas: List[str] = Field(
        default=[],
        description=(
            "Suggested focus areas for news search (e.g., 'Energy sector "
            "strength', 'Banking accumulation')"
        ),
    )
...
class NewsInsight(BaseModel):
    ...
    key_event: str = Field(
        description="Brief description of the main news event (or 'No significant news')"
    )
    event_type: Optional[str] = Field(
        None,
        description=(
            "Category of event: 'Earnings', 'M&A', 'Block Deal', 'SEBI "
            "Action', 'Corporate Action', 'Analyst Rating', 'Sector', "
            "'Macro', 'Circuit', or 'None'"
        ),
    )
    news_date: Optional[str] = Field(
        None,
        description=(
            "Exact date of news event in YYYY-MM-DD format "
            "(if found)"
        ),
    )
    corporate_action: Optional[str] = Field(
        None,
        description=(
            "Corporate action details if applicable (e.g., '2:1 "
            "Bonus', '1:10 Split', 'Rs 5 Dividend')"
        ),
    )
    source: Optional[str] = Field(None, description="News source and date")
    correlation: str = Field(
        description=(
            "How news correlates with price move: 'Strong Confirmation', "
            "'Divergence', or 'Weak'"
        ),
    )
